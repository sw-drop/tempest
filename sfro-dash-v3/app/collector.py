...
class TempestCollector:
    ...
    def start(self):
        self.running = True
        logger.info("Tempest Collector initialized in permanent polling mode.")

    def run_once(self):
        """Executes one polling iteration unconditionally."""
        ...
        logger.info("Executing periodic weather poll...")
        try:
            ...
            try:
                ...
                with urllib.request.urlopen(req_roof, timeout=10) as resp:
                    ...
                    # Find Building 5
                    building_5 = next((b for b in roof_state_raw if str(b.get("device_number")).lstrip('0') == '5'), None)
                    if building_5:
            ...
            if obs_raw.get("status", {}).get("status_code") == 0 and fore_raw.get("status", {}).get("status_code") == 0:
                ...
                # Write atomically to observations.json
                temp_path = os.path.join(self.config.STATIC_DIR, "observations.json.tmp")
                final_path = os.path.join(self.config.STATIC_DIR, "observations.json")
                with open(temp_path, "w") as f:
                    json.dump(processed_data, f)
                os.replace(temp_path, final_path)
                logger.info(f"Successfully wrote observations.json to {final_path}")
            ...
            if timeseries:
                ...
                if sunrise > 0 and sunset > 0:
                    if current_time < sunrise:
                        # It is currently night/early morning BEFORE sunrise (active night)
                        # Show the remaining forecast for this current night
                        filter_start = current_time
                        filter_end = sunrise + 3600
                    elif current_time < (sunset - 3600):
                        # It is daytime (after sunrise, but before tonight's sunset window)
                        # Show tonight's forecast
                        filter_start = sunset - 3600
                        filter_end = sunrise_tomorrow + 3600
                    else:
                        # It is evening/night (after sunset window)
                        # Show the forecast starting from sunset of this night
                        filter_start = sunset - 3600
                        filter_end = sunrise_tomorrow + 3600
                else:
                    filter_start = current_time
