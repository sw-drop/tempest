...
async function fetchFITSImages() {
    try {
        ...
        if (response.ok) {
            ...
            const scopes = ['fra400', '75q'];
            scopes.forEach(scope => {
                const data = metadata[scope] || {};
                const imgEl = document.getElementById(`${scope}-img`);
                ...
                if (titleEl) {
                    titleEl.textContent = data.filename || "Image Loaded";
                }
            });
        }
    } catch (e) {
        console.error("FITS fetch error", e);
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Poll weather observations every 60s
    setInterval(fetchWeather, 60000);
    
    // Poll reports & images metadata every 5 minutes
    setInterval(() => {
        fetchReports();
        fetchFITSImages();
    }, 300000);
    ...
    // Handle resizing for dynamic text boxes
