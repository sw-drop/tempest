...
def update_dashboard(app_dir, agent_dir):
    ...
    schema_path = os.path.join(app_dir, "active_schema.json")
    ...
    if os.path.exists(os.path.join(agent_dir, "custom_skycam.json")):
        save_json(os.path.join(app_dir, "skycam.json"), load_json(os.path.join(agent_dir, "custom_skycam.json")))
    elif schema == "Supper1":
        apod_data = load_json(os.path.join(ROOT_DIR, "apod", "apod.json"), {"url": "./images/apod_fallback.jpg", "title": "APOD"})
        save_json(os.path.join(app_dir, "skycam.json"), {
    ...
    # 2. FRA400 Image
    if os.path.exists(os.path.join(agent_dir, "custom_fra400.json")):
        save_json(os.path.join(app_dir, "fra400.json"), load_json(os.path.join(agent_dir, "custom_fra400.json")))
    elif active_state["fra400"] == "apod":
        apod_data = load_json(os.path.join(ROOT_DIR, "apod", "apod.json"), {"url": "./images/apod_fallback.jpg", "title": "APOD"})
        save_json(os.path.join(app_dir, "fra400.json"), {
            "title": "NASA APOD", "subtitle": apod_data.get("title", ""), "type": "image",
            "data": {"src": apod_data.get("url", ""), "alt": "APOD"}
        })
        ...
        raw = load_json(os.path.join(app_dir, "fra400_raw.json"))
        if raw: 
            save_json(os.path.join(app_dir, "fra400.json"), raw)
        else:
    ...
    if os.path.exists(os.path.join(agent_dir, "custom_75q.json")):
        ...
        raw = load_json(os.path.join(app_dir, "75q_raw.json"))
        if raw: 
            save_json(os.path.join(app_dir, "75q.json"), raw)
        else:
            # FALLBACK: No images yet
            q75_forecast = load_json(os.path.join(app_dir, "nina_75q.json"))
            if q75_forecast:
                # Rename the title to match the image state requirement
                q75_forecast["title"] = "Observatory Forecast"
                save_json(os.path.join(app_dir, "75q.json"), q75_forecast)

    # 4. Roofbox Card (Discord Overrides)
    if os.path.exists(os.path.join(agent_dir, "custom_roof.json")):
        save_json(os.path.join(app_dir, "roof.json"), load_json(os.path.join(agent_dir, "custom_roof.json")))
    else:
        ...
        if "Closed" in schema or schema in ["Afternoon1", "Supper1", "RoofOpenEvening1"]:
            ...
            if "data" in roof_data:
                ...
                q75_text = load_json(os.path.join(app_dir, "nina_75q.json"), {}).get("data", {}).get("text", "")
    ...
    if os.path.exists(os.path.join(agent_dir, "custom_fra400cap.json")):
        save_json(os.path.join(app_dir, "fra400cap.json"), load_json(os.path.join(agent_dir, "custom_fra400cap.json")))
    elif active_state.get("captures") == "rates":
        rates_left = load_json(os.path.join(app_dir, "rates_left.json"))
        if rates_left: save_json(os.path.join(app_dir, "fra400cap.json"), rates_left)
    else:
        nina_fra = load_json(os.path.join(app_dir, "nina_fra400.json"))
        if nina_fra: save_json(os.path.join(app_dir, "fra400cap.json"), nina_fra)

    if os.path.exists(os.path.join(agent_dir, "custom_q75cap.json")):
        save_json(os.path.join(app_dir, "q75cap.json"), load_json(os.path.join(agent_dir, "custom_q75cap.json")))
    elif active_state.get("captures") == "rates":
        rates_right = load_json(os.path.join(app_dir, "rates_right.json"))
        if rates_right: save_json(os.path.join(app_dir, "q75cap.json"), rates_right)
    else:
        nina_75q = load_json(os.path.join(app_dir, "nina_75q.json"))
        if nina_75q: save_json(os.path.join(app_dir, "q75cap.json"), nina_75q)

def main():
    parser = argparse.ArgumentParser(description="Master controller to route dashboard cards based on schema.")
    parser.add_argument("--app-dir", default="v5-test/app_data", help="System output directory for JSON files")
    parser.add_argument("--agent-dir", default="v5-test/data", help="Agent input directory")
    args = parser.parse_args()
    update_dashboard(args.app_dir, args.agent_dir)
