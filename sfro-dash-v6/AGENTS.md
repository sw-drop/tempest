...
Sync & Deployment:
The remote server is Pi5-1. On the remote server, the deployment of the dashboard is currently at /docker/sfro-dash-v5. As part of this project, we may change that to /docker/sfro-dash-v6.
Use only simple .sh scripts with scp or rsync (or similar) to push changes to the server.
There is no automated sync to the server. The project directory structure on the Mac my have sync type naming, but this should not be interpreted as a capability.
